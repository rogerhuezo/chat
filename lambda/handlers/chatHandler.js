...
const handleChat = async (event) => {
  ...
  // If Lex classified as FallbackIntent but another intent has high confidence,
  // re-route to avoid losing clear user intent buried in verbose messages.
  if (intent === 'FallbackIntent' && event.interpretations?.length) {
    const alternatives = event.interpretations
      .filter(i => i.intent?.name !== 'FallbackIntent' && (i.nluConfidence || 0) > 0.6);
    if (alternatives.length) {
      const best = alternatives.sort((a, b) => (b.nluConfidence || 0) - (a.nluConfidence || 0))[0];
  ...
  if ((intent === 'FallbackIntent' || !intent) && isCatalogRequest(msgLower, slots)) {
    ...
    // catalogHandler returned null (break/fix detected) — fall through to KB
  }
